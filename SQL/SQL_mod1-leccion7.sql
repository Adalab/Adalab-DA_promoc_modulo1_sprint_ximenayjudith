...
SELECT MIN(unit_price) AS lowestPrice, MAX(unit_price) highestPrice
FROM products;


/* 2 Conociendo el numero de productos y su precio medio:
Adicionalmente nos piden que diseñemos otra consulta para conocer el número de productos y el precio medio 
de todos ellos (en general, no por cada producto).*/
...
SELECT MAX(freight) AS cargaMaxima, MIN(freight) AS cargaMinima
FROM orders
WHERE ship_country = 'UK';


/*SELECT MIN (freight) AS CargaMinima, MAX (freight) AS CargaMaxima
FROM  orders
WHERE ship_country = UK;
...
dicho número es demasiado elevado. 
...
salgan demasiados resultados, nos piden que los limitemos a los 10 con ID más elevado, que serán los más recientes. 
